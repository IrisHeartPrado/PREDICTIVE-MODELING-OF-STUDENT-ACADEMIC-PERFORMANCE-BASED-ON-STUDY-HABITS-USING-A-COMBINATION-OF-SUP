...
def predict_new_data(meta_model, rf_model, svm_model, test_data_path, preprocessor, true_labels_path=None):
    try:
        ...
        # If true labels are provided, evaluate the performance
        if true_labels_path:
            true_labels = pd.read_csv(true_labels_path)['Status']
            
            # Map labels if necessary
            label_mapping = {'Regular': 1, 'Irregular': 0}  # Example mapping
            true_labels = true_labels.replace(label_mapping).values  # Convert labels
            
            print("Mapped true labels:", true_labels)  # Debugging output
...
input_notebook = 'stacking_model_2.ipynb'  # Replace with the path to your notebook
output_script = 'stacking_model.py'  # Desired name for the Python script
